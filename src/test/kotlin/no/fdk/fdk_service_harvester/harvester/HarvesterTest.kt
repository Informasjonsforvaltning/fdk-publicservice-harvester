...
@Tag("unit")
class HarvesterTest {
    private val servicesRepository: ServicesRepository = mock()
    ...
    private val harvester = ServicesHarvester(
        adapter, fusekiAdapter, servicesRepository, miscRepository, valuesMock
    )
    ...
    fun harvestDataSourceSavedWhenDBIsEmpty() {
        ...
        whenever(valuesMock.serviceHarvesterUri)
            .thenReturn("http://localhost:5000/services")
        ...
        argumentCaptor<List<ServiceDBO>>().apply {
            verify(servicesRepository, times(1)).saveAll(capture())
    ...
    fun harvestDataSourceNotPersistedWhenNoChangesFromDB() {
        ...
        argumentCaptor<List<ServiceDBO>>().apply {
            verify(servicesRepository, times(0)).saveAll(capture())
        }
    ...
    fun onlyRelevantUpdatedWhenHarvestedFromDB() {
        ...
        whenever(servicesRepository.findById(SERVICE_DBO_0.uri))
        ...
        whenever(servicesRepository.findById(SERVICE_DBO_1.uri))
            .thenReturn(Optional.of(SERVICE_DBO_1))
        whenever(servicesRepository.findById(SERVICE_DBO_2.uri))
            .thenReturn(Optional.of(SERVICE_DBO_2))

        val expectedServiceDBO = SERVICE_DBO_0.copy(
        ...
        argumentCaptor<List<ServiceDBO>>().apply {
            ...
            firstValue.first().printTurtleDiff(expectedServiceDBO)
            assertEquals(expectedServiceDBO, firstValue.first())
        }
