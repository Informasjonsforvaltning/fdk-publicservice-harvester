package no.fdk.fdk_publicservice_harvester.contract
...
class PublicServicesTest: ApiTestContext() {
    ...
    @Test
    fun findAll() {
        val response = apiGet("/publicservices", "text/turtle", port)
    ...
    @Test
    fun findSpecific() {
        val response = apiGet("/publicservices/$SERVICE_ID_0", "application/rdf+json", port)
        Assumptions.assumeTrue(HttpStatus.OK.value() == response["status"])
    ...
    @Test
    fun idDoesNotExist() {
        val response = apiGet("/publicservices/123", "text/turtle", port)
