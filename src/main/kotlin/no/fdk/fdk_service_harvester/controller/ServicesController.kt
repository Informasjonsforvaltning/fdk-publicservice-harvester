...
import no.fdk.fdk_service_harvester.rdf.jenaTypeFromAcceptHeader
...
@Controller
@RequestMapping(value = ["/services"], produces = ["text/turtle", "text/n3", "application/rdf+json", "application/ld+json", "application/rdf+xml", "application/n-triples"])
open class ServicesController(private val servicesService: ServicesService) {
    ...
    fun getServiceById(httpServletRequest: HttpServletRequest, @PathVariable id: String): ResponseEntity<String> {
        ...
        return if (returnType == JenaType.NOT_JENA) ResponseEntity(HttpStatus.NOT_ACCEPTABLE)
            ...
            servicesService.getServiceById(id, returnType ?: JenaType.TURTLE)
    ...
    fun getServices(httpServletRequest: HttpServletRequest): ResponseEntity<String> {
        ...
        return if (returnType == JenaType.NOT_JENA) ResponseEntity(HttpStatus.NOT_ACCEPTABLE)
        else ResponseEntity(servicesService.getAll(returnType ?: JenaType.TURTLE), HttpStatus.OK)
    }
